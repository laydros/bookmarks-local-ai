...
def test_suggest_categories_basic(sample_bookmarks):
    ...
    with patch.object(suggester, "_cluster_embeddings", return_value=[0, 0, 0, 1, 1, 1]), patch(
...
def test_suggest_categories_with_kmeans(sample_bookmarks):
    ...
    with patch("sklearn.cluster.KMeans") as mock_kmeans, patch(
        "core.category_suggester.ProgressTracker"
    ), patch.object(
        suggester,
        "_generate_cluster_summary",
        side_effect=[
            {"name": "Tech Resources", "description": "Technology-related bookmarks"},
