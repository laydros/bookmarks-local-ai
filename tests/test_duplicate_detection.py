...
def test_duplicate_detection_by_url(tmp_path):
    ...
    existing_bookmarks = [
        {"url": "https://example.com", "title": "Example Site", "description": "A test site"}
    ]
    target_file = existing_dir / "existing.json"
    BookmarkLoader.save_to_file([Bookmark.from_dict(b) for b in existing_bookmarks], str(target_file))
    ...
    with patch.object(BookmarkImporter, "print_summary"):
        with patch("bookmark_importer.WebExtractor.is_valid_url", return_value=True):
...
def test_skip_duplicate_check_option(tmp_path):
    ...
    with patch.object(BookmarkImporter, "print_summary"):
        with patch("bookmark_importer.WebExtractor.is_valid_url", return_value=True), patch(
            ...
            dead, duplicates = importer.import_from_file(str(new_file), check_duplicates=False)
